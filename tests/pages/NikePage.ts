...
export class NikePage {
  ...
  async fillEmail(email: string) {
    ...
    // active parfois le bouton Continuer
  ...
  async selectFirstAvailableSizeIfPresent() {
    ...
    // Pas de tailles -> produit sans tailles
    if (!hasSize) return;
  ...
  async addToCart() {
    const addBtn = this.page.getByRole('button', { name: /Ajouter au panier|Ajouter au sac/i }).first();
